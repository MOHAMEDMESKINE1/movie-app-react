...
function Blog() {
    ...
    const fetchAllMovies =async () =>{
        ...
        try {
            const allMovies = await useFetch(`https://api.themoviedb.org/3/trending/movie/day?api_key=${apiKey}`)
            setMovies(allMovies)
    ...
    const fetchTvShows =async () =>{
        try{
            ...
            const allTvshows = await useFetch(`https://api.themoviedb.org/3/trending/tv/day?api_key=${apiKey}`);
            setTvShows(allTvshows);
           
          
        } catch (error) {
            console.error('Error fetching tvhsows:', error);
        }
    }
    const fetchTopRated =async () =>{
        try{

            const TopRated = await useFetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}`);
        ...
        <div>
            <section className=" m-5">
                    ...
                    <div className="container">
                        <h1 className="text-gray-900  my-3 text-2xl ">TOP 4 TRENDINGS TVSHOWS </h1>
                        <hr className="bg-gray-50" />
